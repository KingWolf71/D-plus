...
Module LJView
   ...
   Structure stCell
      ...
      seed.i                    ; Per-cell seed for all derived properties
      color.i                   ; Background color (derived from seed + palette)
      locked.i                  ; If true, Random skips this cell entirely
   ...
   ; Forward declarations (used by debug panel)
   Declare.i GenerateColor(seed.i, index.i)
   Declare.i HSLtoRGB(h.f, s.f, l.f)
   Declare.i GeneratePaletteColor(viewSeed.i, cellSeed.i, role.i = 0)
   ...
   CompilerIf (Defined(PB_Compiler_Debugger, #PB_Constant) And #PB_Compiler_Debugger) Or #LJV_DEBUG
      ...
      ; Color presets
      #DEBUG_BTN_PRESET_1 = 9083       ; White
      #DEBUG_BTN_PRESET_2 = 9084       ; Light gray
      #DEBUG_BTN_PRESET_3 = 9085       ; Red
      #DEBUG_BTN_PRESET_4 = 9086       ; Green
      #DEBUG_BTN_PRESET_5 = 9087       ; Blue
      #DEBUG_BTN_PRESET_6 = 9088       ; Yellow
      #DEBUG_BTN_COPY_CODE = 9089      ; Copy code button
      #DEBUG_COMBO_TEMPLATE = 9090    ; View templates dropdown
      ...
      Global gCopyFeedbackTimer.i = 0    ; Timer for "Copied!" feedback reset
      ...
      Procedure UpdateCellPropsDisplay()
         ...
         If gSelectStartCol >= 0
            ...
            *cell = @gView\cells(idx)
            ...
            SetGadgetState(#DEBUG_CHK_COLORLK, *cell\locked)
      ...
      Procedure ShowDebugPanel()
         ...
         If gDebugWindow
            ...
            ; Template dropdown
            TextGadget(#PB_Any, 10, y + 3, 60, 20, "Template:")
            ComboBoxGadget(#DEBUG_COMBO_TEMPLATE, 70, y, 240, 24)
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "(Custom)")
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "Login Screen")
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "Chat App")
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "Settings Menu")
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "Dashboard")
            AddGadgetItem(#DEBUG_COMBO_TEMPLATE, -1, "Data Entry Form")
            SetGadgetState(#DEBUG_COMBO_TEMPLATE, 0)
            y + 28
            ...
            TextGadget(#PB_Any, 8, cy + 4, 42, 18, "Quick:")
            ButtonGadget(#DEBUG_BTN_PRESET_1, 52, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_1, #PB_Gadget_BackColor, RGB(255, 255, 255))
            ButtonGadget(#DEBUG_BTN_PRESET_2, 78, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_2, #PB_Gadget_BackColor, RGB(180, 180, 180))
            ButtonGadget(#DEBUG_BTN_PRESET_3, 104, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_3, #PB_Gadget_BackColor, RGB(220, 80, 80))
            ButtonGadget(#DEBUG_BTN_PRESET_4, 130, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_4, #PB_Gadget_BackColor, RGB(80, 180, 80))
            ButtonGadget(#DEBUG_BTN_PRESET_5, 156, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_5, #PB_Gadget_BackColor, RGB(80, 120, 200))
            ButtonGadget(#DEBUG_BTN_PRESET_6, 182, cy, 24, 20, "")
            SetGadgetColor(#DEBUG_BTN_PRESET_6, #PB_Gadget_BackColor, RGB(230, 200, 80))
            CloseGadgetList()
            y + #SEC_HEIGHT_GADGET
            ...
            y + 4
            hdrGadget = TextGadget(#PB_Any, 0, y, 250, 22, "  GENERATED CODE")
            SetGadgetColor(hdrGadget, #PB_Gadget_BackColor, #CLR_HEADER_BG)
            SetGadgetColor(hdrGadget, #PB_Gadget_FrontColor, #CLR_HEADER_FG)
            ButtonGadget(#DEBUG_BTN_COPY_CODE, 252, y + 1, 66, 20, "Copy")
            y + 24
      ...
      Procedure ApplySeedChange()
         ; Read seed from input and refresh view (changes palette, regenerates colors)
         ...
         If gDebugWindow And IsWindow(gDebugWindow)
            ...
            If newSeed <> gView\seed
               gView\seed = newSeed
               ; Regenerate cell colors with new palette (skip locked cells)
               For i = 0 To gView\totalCells - 1
                  If Not gView\cells(i)\locked
                     ; Keep cell seed, just regenerate color with new palette
                     If gView\cells(i)\seed = 0 : gView\cells(i)\seed = i * 2654435761 : EndIf
                     gView\cells(i)\color = GeneratePaletteColor(gView\seed, gView\cells(i)\seed, 0)
                  EndIf
               Next
               ; Refresh view
               ViewShow(gView\width, gView\height)
      ...
      Procedure ApplyTemplate(templateID.i)
         ; Apply a view template - sets up common layouts
         Protected i.i, idx.i, col.i, row.i
         Protected maxDim.i = gView\cols
         If gView\rows > maxDim : maxDim = gView\rows : EndIf

         ; Clear all gadgets first
         For i = 0 To gView\totalCells - 1
            gView\cells(i)\gadgetCount = 0
            gView\cells(i)\joinGroup = 0
            gView\cells(i)\isMaster = #False
            gView\cells(i)\locked = #False
         ...
         Select templateID
            Case 1  ; Login Screen
               ; Title at top
               idx = GetCellIndex(0, 0)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
               gView\cells(idx)\gadgets(0)\text = "Welcome"
               gView\cells(idx)\gadgets(0)\align = #LJV_CENTER
               gView\cells(idx)\gadgets(0)\size = 2  ; Large
               gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 100, 3)  ; Header color

               ; Username row (row 2)
               idx = GetCellIndex(1, 2)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_INPUT
               gView\cells(idx)\gadgets(0)\text = "Username"
               gView\cells(idx)\gadgets(0)\align = #LJV_LEFT

               ; Password row (row 3)
               idx = GetCellIndex(1, 3)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_INPUT
               gView\cells(idx)\gadgets(0)\text = "Password"
               gView\cells(idx)\gadgets(0)\align = #LJV_LEFT

               ; Login button (row 4)
               idx = GetCellIndex(1, 4)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_BUTTON
               gView\cells(idx)\gadgets(0)\text = "Login"
               gView\cells(idx)\gadgets(0)\align = #LJV_CENTER
               gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 200, 1)  ; Accent

               ; Close button top-right
               If gView\cols >= 4
                  idx = GetCellIndex(3, 0)
                  gView\cells(idx)\gadgetCount = 1
                  gView\cells(idx)\gadgets(0)\type = #LJV_BUTTON
                  gView\cells(idx)\gadgets(0)\text = "X"
                  gView\cells(idx)\gadgets(0)\align = #LJV_RIGHT | #LJV_TOP
                  gView\cells(idx)\lockCorner = #True
               EndIf
            ...
            Case 2  ; Chat App
               ...
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
               gView\cells(idx)\gadgets(0)\text = "Chat"
               gView\cells(idx)\gadgets(0)\align = #LJV_LEFT
               gView\cells(idx)\gadgets(0)\size = 2
               gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 100, 3)

               ; Message area (middle rows) - scroll container
               idx = GetCellIndex(0, 2)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_SCROLLAREA
               gView\cells(idx)\gadgets(0)\text = "Messages"
               gView\cells(idx)\gadgets(0)\align = #LJV_CENTER

               ; Input row (bottom)
               If gView\rows >= 5
                  idx = GetCellIndex(0, gView\rows - 2)
                  gView\cells(idx)\gadgetCount = 1
                  gView\cells(idx)\gadgets(0)\type = #LJV_INPUT
                  gView\cells(idx)\gadgets(0)\text = "Type message..."
                  gView\cells(idx)\gadgets(0)\align = #LJV_LEFT

                  ; Send button
                  idx = GetCellIndex(gView\cols - 1, gView\rows - 2)
                  gView\cells(idx)\gadgetCount = 1
                  gView\cells(idx)\gadgets(0)\type = #LJV_BUTTON
                  gView\cells(idx)\gadgets(0)\text = "Send"
                  gView\cells(idx)\gadgets(0)\align = #LJV_CENTER
                  gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 300, 1)
               EndIf
            ...
            Case 3  ; Settings Menu
               ...
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
               gView\cells(idx)\gadgets(0)\text = "Settings"
               gView\cells(idx)\gadgets(0)\align = #LJV_LEFT
               gView\cells(idx)\gadgets(0)\size = 2
               gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 100, 3)

               ; Setting rows with checkboxes
               Protected settingNames.s = "Notifications|Dark Mode|Sound|Auto-save|Sync"
               Protected settingCount.i = 5
               For i = 0 To settingCount - 1
                  If i + 1 < gView\rows
                     ; Label
                     idx = GetCellIndex(0, i + 1)
                     gView\cells(idx)\gadgetCount = 1
                     gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
                     gView\cells(idx)\gadgets(0)\text = StringField(settingNames, i + 1, "|")
                     gView\cells(idx)\gadgets(0)\align = #LJV_LEFT

                     ; Toggle
                     If gView\cols >= 3
                        idx = GetCellIndex(gView\cols - 1, i + 1)
                        gView\cells(idx)\gadgetCount = 1
                        gView\cells(idx)\gadgets(0)\type = #LJV_CHECKBOX
                        gView\cells(idx)\gadgets(0)\text = ""
                        gView\cells(idx)\gadgets(0)\align = #LJV_RIGHT
                     EndIf
                  EndIf
               Next

            Case 4  ; Dashboard
               ; Create cards in 2x2 or similar grid
               Protected cardLabels.s = "Users|Revenue|Orders|Traffic"
               Protected cardIdx.i = 0
               For row = 0 To 1
                  For col = 0 To 1
                     If col < gView\cols And row < gView\rows
                        idx = GetCellIndex(col, row)
                        gView\cells(idx)\gadgetCount = 1
                        gView\cells(idx)\gadgets(0)\type = #LJV_CONTAINER
                        gView\cells(idx)\gadgets(0)\text = StringField(cardLabels, cardIdx + 1, "|")
                        gView\cells(idx)\gadgets(0)\align = #LJV_CENTER
                        gView\cells(idx)\color = GeneratePaletteColor(gView\seed, cardIdx * 100, 0)
                        cardIdx + 1
                     EndIf
                  Next
               Next

            Case 5  ; Data Entry Form
               ; Form title
               idx = GetCellIndex(0, 0)
               gView\cells(idx)\gadgetCount = 1
               gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
               gView\cells(idx)\gadgets(0)\text = "Data Entry"
               gView\cells(idx)\gadgets(0)\align = #LJV_LEFT
               gView\cells(idx)\gadgets(0)\size = 2
               gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 100, 3)

               ; Form fields
               Protected fieldLabels.s = "Name|Email|Phone|Address"
               For i = 0 To 3
                  If i + 1 < gView\rows
                     ; Label
                     idx = GetCellIndex(0, i + 1)
                     gView\cells(idx)\gadgetCount = 1
                     gView\cells(idx)\gadgets(0)\type = #LJV_TEXT
                     gView\cells(idx)\gadgets(0)\text = StringField(fieldLabels, i + 1, "|")
                     gView\cells(idx)\gadgets(0)\align = #LJV_RIGHT

                     ; Input
                     If gView\cols >= 2
                        idx = GetCellIndex(1, i + 1)
                        gView\cells(idx)\gadgetCount = 1
                        gView\cells(idx)\gadgets(0)\type = #LJV_INPUT
                        gView\cells(idx)\gadgets(0)\text = ""
                        gView\cells(idx)\gadgets(0)\align = #LJV_LEFT
               ...
               If gView\rows >= 6
                  idx = GetCellIndex(1, 5)
                  gView\cells(idx)\gadgetCount = 1
                  gView\cells(idx)\gadgets(0)\type = #LJV_BUTTON
                  gView\cells(idx)\gadgets(0)\text = "Submit"
                  gView\cells(idx)\gadgets(0)\align = #LJV_CENTER
                  gView\cells(idx)\color = GeneratePaletteColor(gView\seed, 200, 1)
               EndIf
         EndSelect
      ...
      Procedure HandleDebugEvent(event.i, eventWindow.i, eventGadget.i)
         ...
         ; Reset copy button feedback after delay
         If gCopyFeedbackTimer > 0 And (ElapsedMilliseconds() - gCopyFeedbackTimer) > 1500
            If IsGadget(#DEBUG_BTN_COPY_CODE)
               SetGadgetText(#DEBUG_BTN_COPY_CODE, "Copy")
            EndIf
            gCopyFeedbackTimer = 0
         EndIf
            ...
            Select event
               ...
               Case #PB_Event_Gadget
                  Select eventGadget
                     ...
                     Case #DEBUG_COMBO_TEMPLATE
                        ; Template selection changed
                        Protected templateState.i = GetGadgetState(#DEBUG_COMBO_TEMPLATE)
                        If templateState > 0
                     ...
                     Case #DEBUG_BTN_RANDOM
                        ; Random works on selection if exists, otherwise all unlocked cells
                        Protected hasSelection.i = Bool(gSelectStartCol >= 0 And gSelectEndCol >= 0)
                        Protected rMinCol.i, rMaxCol.i, rMinRow.i, rMaxRow.i
                        Protected rCol.i, rRow.i, rIdx.i

                        ; Generate new view seed (determines palette)
                        randomSeed = Random(999999)
                        gView\seed = randomSeed
                        SetGadgetText(#DEBUG_INPUT_SEED, Str(randomSeed))

                        If hasSelection
                           ; Randomize only selected cells
                           rMinCol = gSelectStartCol : rMaxCol = gSelectEndCol
                           If rMaxCol < rMinCol : Swap rMinCol, rMaxCol : EndIf
                           rMinRow = gSelectStartRow : rMaxRow = gSelectEndRow
                           If rMaxRow < rMinRow : Swap rMinRow, rMaxRow : EndIf

                           For rRow = rMinRow To rMaxRow
                              For rCol = rMinCol To rMaxCol
                                 rIdx = GetCellIndex(rCol, rRow)
                                 If Not gView\cells(rIdx)\locked
                                    ; Generate new cell seed
                                    gView\cells(rIdx)\seed = Random(999999)
                                    gView\cells(rIdx)\color = GeneratePaletteColor(gView\seed, gView\cells(rIdx)\seed, 0)
                                 EndIf
                              Next
                           Next
                        Else
                           ...
                           For i = 0 To gView\totalCells - 1
                              If Not gView\cells(i)\locked
                                 ; Generate new cell seed
                                 gView\cells(i)\seed = Random(999999)
                                 gView\cells(i)\color = GeneratePaletteColor(gView\seed, gView\cells(i)\seed, 0)
                              EndIf
                           Next
                        EndIf
                     ...
                     Case #DEBUG_CHK_COLORLK
                        ...
                        If gSelectStartCol >= 0
                           ...
                           gView\cells(clIdx)\locked = GetGadgetState(#DEBUG_CHK_COLORLK)
                        EndIf
                        ProcedureReturn #True
                     ...
                     Case #DEBUG_BTN_PRESET_1, #DEBUG_BTN_PRESET_2, #DEBUG_BTN_PRESET_3, #DEBUG_BTN_PRESET_4, #DEBUG_BTN_PRESET_5, #DEBUG_BTN_PRESET_6
                        ; Color preset buttons - apply to selected cell
                        If gSelectStartCol >= 0
                           Protected prIdx.i = GetCellIndex(gSelectStartCol, gSelectStartRow)
                           Protected presetColor.i
                           Select EventGadget()
                              Case #DEBUG_BTN_PRESET_1 : presetColor = RGB(255, 255, 255)  ; White
                              Case #DEBUG_BTN_PRESET_2 : presetColor = RGB(180, 180, 180)  ; Light gray
                              Case #DEBUG_BTN_PRESET_3 : presetColor = RGB(220, 80, 80)    ; Red
                              Case #DEBUG_BTN_PRESET_4 : presetColor = RGB(80, 180, 80)    ; Green
                              Case #DEBUG_BTN_PRESET_5 : presetColor = RGB(80, 120, 200)   ; Blue
                              Case #DEBUG_BTN_PRESET_6 : presetColor = RGB(230, 200, 80)   ; Yellow
                           EndSelect
                           gView\cells(prIdx)\color = presetColor
                           SetGadgetColor(#DEBUG_BTN_COLOR, #PB_Gadget_BackColor, presetColor)
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                        ProcedureReturn #True

                     Case #DEBUG_BTN_COPY_CODE
                        ; Copy code to clipboard with visual feedback
                        Protected codeText.s = GetGadgetText(#DEBUG_EDITOR_CODE)
                        SetClipboardText(codeText)
                        ; Visual feedback - change button text briefly
                        SetGadgetText(#DEBUG_BTN_COPY_CODE, "Copied!")
                        ; Schedule text reset via delayed redraw
                        gCopyFeedbackTimer = ElapsedMilliseconds()
                        ProcedureReturn #True
                  EndSelect
            EndSelect
         EndIf
   ...
   Procedure.i HSLtoRGB(h.f, s.f, l.f)
   ...
   Procedure.i GeneratePaletteColor(viewSeed.i, cellSeed.i, role.i = 0)
      ; Generate harmonious color from view palette + cell variation
      ; role: 0=background, 1=accent, 2=text-bg, 3=header
      Protected baseHue.f, saturation.f, lightness.f
      Protected hueOffset.f, lightOffset.f

      ; View seed determines the base hue (0-360)
      RandomSeed(viewSeed)
      baseHue = Random(360)

      ; Cell seed determines variation within palette
      RandomSeed(cellSeed)
      hueOffset = Random(40) - 20        ; ±20 degrees for harmony
      lightOffset = Random(20) - 10      ; ±10% lightness variation

      ; Set saturation and lightness based on role
      Select role
         Case 0  ; Background - soft, light colors
            saturation = 0.35 + Random(15) / 100.0
            lightness = 0.75 + lightOffset / 100.0
         Case 1  ; Accent - more saturated
            saturation = 0.55 + Random(20) / 100.0
            lightness = 0.55 + lightOffset / 100.0
         Case 2  ; Text background - neutral
            saturation = 0.15 + Random(10) / 100.0
            lightness = 0.88 + lightOffset / 200.0
         Case 3  ; Header - darker
            saturation = 0.45 + Random(15) / 100.0
            lightness = 0.45 + lightOffset / 100.0
         Default
            saturation = 0.35
            lightness = 0.75
      EndSelect

      ; Apply hue with offset, keep in 0-360 range
      Protected finalHue.f = baseHue + hueOffset
      If finalHue < 0 : finalHue + 360 : EndIf
      If finalHue >= 360 : finalHue - 360 : EndIf

      ProcedureReturn HSLtoRGB(finalHue, saturation, lightness)
   EndProcedure

   Procedure.i GenerateColor(seed.i, index.i)
      ; Generate deterministic color using palette system
      ; seed = view seed, index = cell index (used as cell seed if cell has no seed)
      Protected cellSeed.i = index * 2654435761  ; Knuth hash for variety
      ProcedureReturn GeneratePaletteColor(seed, cellSeed, 0)
   EndProcedure
   ...
   Procedure   RenderCell(idx.i, baseX.i, baseY.i, cellW.i, cellH.i)
      ...
      CompilerIf (Defined(PB_Compiler_Debugger, #PB_Constant) And #PB_Compiler_Debugger) Or #LJV_DEBUG
         ; Use cell's actual col/row (not computed from position which can be shifted)
         visualCol = *cell\col
         visualRow = *cell\row
      ...
      ; Show gadget count indicator if multiple gadgets (×2, ×3, etc.)
      If *cell\gadgetCount > 1
         Protected countLabel.i = TextGadget(#PB_Any, actualW - 22, 2, 20, 14, Chr($D7) + Str(*cell\gadgetCount))
         SetGadgetColor(countLabel, #PB_Gadget_BackColor, RGB(60, 60, 60))
         SetGadgetColor(countLabel, #PB_Gadget_FrontColor, RGB(255, 255, 255))
         SetGadgetFont(countLabel, LoadFont(#PB_Any, "Segoe UI", 7, #PB_Font_Bold))
      EndIf
