...
Module LJView
   ...
   CompilerIf (Defined(PB_Compiler_Debugger, #PB_Constant) And #PB_Compiler_Debugger) Or #LJV_DEBUG
      ...
      Procedure ShowDebugPanel()
         ...
         gDebugWindow = OpenWindow(#PB_Any, 10, 10, 320, 580, "View Debug Controls",
                                    #PB_Window_SystemMenu | #PB_Window_Tool)
         If gDebugWindow
            y = 10

            ; Row 1: Seed controls
            TextGadget(#DEBUG_LBL_SEED, 10, y + 5, 40, 20, "Seed:")
            StringGadget(#DEBUG_INPUT_SEED, 55, y, 80, 26, Str(gView\seed))
            ButtonGadget(#DEBUG_BTN_RANDOM, 140, y, 50, 26, "Rnd")
            ButtonGadget(#DEBUG_BTN_ROTATE, 200, y, 110, 26, "Rotate 90")
            y + 35

            ; Row 2: Orientation display
            If gView\orientation = 0
               TextGadget(#DEBUG_LBL_ORIENT, 10, y, 300, 20, "Orientation: Portrait (" + Str(gView\width) + "x" + Str(gView\height) + ")")
            Else
               TextGadget(#DEBUG_LBL_ORIENT, 10, y, 300, 20, "Orientation: Landscape (" + Str(gView\width) + "x" + Str(gView\height) + ")")
            EndIf
            y + 28

            ; Row 3: Cell selection status
            TextGadget(#DEBUG_LBL_SELECT, 10, y, 300, 20, "Click cell to set start")
            y + 24

            ; Row 4: Cell selection and merge
            TextGadget(#PB_Any, 10, y + 5, 40, 20, "Start:")
            StringGadget(#DEBUG_INPUT_START, 55, y, 50, 26, "")
            SetGadgetColor(#DEBUG_INPUT_START, #PB_Gadget_BackColor, RGB(240, 240, 240))
            TextGadget(#PB_Any, 115, y + 5, 30, 20, "End:")
            StringGadget(#DEBUG_INPUT_END, 150, y, 50, 26, "")
            SetGadgetColor(#DEBUG_INPUT_END, #PB_Gadget_BackColor, RGB(240, 240, 240))
            y + 35

            ; Row 5: Merge/Unmerge buttons
            ButtonGadget(#DEBUG_BTN_MERGE, 10, y, 90, 26, "Merge")
            DisableGadget(#DEBUG_BTN_MERGE, #True)
            ButtonGadget(#DEBUG_BTN_UNMERGE, 105, y, 90, 26, "Unmerge")
            DisableGadget(#DEBUG_BTN_UNMERGE, #True)
            y + 35

            ; Cell Properties Section
            FrameGadget(#DEBUG_FRAME_CELL, 10, y, 300, 160, "Cell Properties")
            y + 20

            ; Size options
            TextGadget(#DEBUG_LBL_SIZE, 20, y, 35, 20, "Size:")
            OptionGadget(#DEBUG_OPT_SIZE_S, 60, y, 35, 20, "S")
            OptionGadget(#DEBUG_OPT_SIZE_M, 100, y, 35, 20, "M")
            OptionGadget(#DEBUG_OPT_SIZE_L, 140, y, 35, 20, "L")
            OptionGadget(#DEBUG_OPT_SIZE_XL, 180, y, 40, 20, "XL")
            SetGadgetState(#DEBUG_OPT_SIZE_M, #True)  ; Default to Medium
            y + 24

            ; Style options
            CheckBoxGadget(#DEBUG_CHK_BOLD, 20, y, 60, 20, "Bold")
            CheckBoxGadget(#DEBUG_CHK_ITALIC, 85, y, 60, 20, "Italic")
            y + 24

            ; Nudge options
            TextGadget(#DEBUG_LBL_NUDGE, 20, y, 45, 20, "Nudge:")
            CheckBoxGadget(#DEBUG_CHK_NUDGE_L, 70, y, 28, 20, "<")
            CheckBoxGadget(#DEBUG_CHK_NUDGE_R, 100, y, 28, 20, ">")
            CheckBoxGadget(#DEBUG_CHK_NUDGE_U, 135, y, 28, 20, "^")
            CheckBoxGadget(#DEBUG_CHK_NUDGE_D, 165, y, 28, 20, "v")
            y + 24

            ; Cell flags
            CheckBoxGadget(#DEBUG_CHK_CORNER, 20, y, 90, 20, "Lock Corner")
            CheckBoxGadget(#DEBUG_CHK_VISIBLE, 115, y, 70, 20, "Visible")
            SetGadgetState(#DEBUG_CHK_VISIBLE, #True)  ; Default visible
            y + 24

            ; Color controls
            ButtonGadget(#DEBUG_BTN_COLOR, 20, y, 70, 24, "Color")
            CheckBoxGadget(#DEBUG_CHK_COLORLK, 100, y + 2, 90, 20, "Lock Color")
            y + 30

            ; Event controls
            TextGadget(#DEBUG_LBL_EVENTS, 20, y, 50, 20, "Events:")
            CheckBoxGadget(#DEBUG_CHK_EVT_TAP, 70, y, 40, 20, "Tap")
            CheckBoxGadget(#DEBUG_CHK_EVT_SWIPE, 115, y, 50, 20, "Swipe")
            CheckBoxGadget(#DEBUG_CHK_EVT_DRAG, 170, y, 45, 20, "Drag")
            y + 22
            CheckBoxGadget(#DEBUG_CHK_EVT_LONG, 70, y, 80, 20, "LongPress")
            y + 24
            TextGadget(#DEBUG_LBL_HANDLER, 20, y + 2, 50, 20, "Handler:")
            StringGadget(#DEBUG_INPUT_HANDLER, 70, y, 120, 22, "")
            y + 32

            ; Code display (auto-copied to clipboard on change)
            TextGadget(#PB_Any, 10, y, 200, 20, "Generated Code (auto-copied):")
            y + 22
            EditorGadget(#DEBUG_EDITOR_CODE, 10, y, 300, 140, #PB_Editor_ReadOnly)

            ; Initial code generation
            UpdateCodeDisplay()
            DisableCellProps(#True)  ; Disabled until cell selected
         EndIf
      EndProcedure
                  ...
                  Select eventGadget
                     ...
                     Case #DEBUG_OPT_SIZE_S, #DEBUG_OPT_SIZE_M, #DEBUG_OPT_SIZE_L, #DEBUG_OPT_SIZE_XL
                        ...
                        If gSelectStartCol >= 0
                           ...
                           If gView\cells(sIdx)\gadgetCount > 0
                              gView\cells(sIdx)\gadgets(0)\size = sizeVal
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_BOLD, #DEBUG_CHK_ITALIC
                        ...
                        If gSelectStartCol >= 0
                           ...
                           If gView\cells(stIdx)\gadgetCount > 0
                              gView\cells(stIdx)\gadgets(0)\bold = GetGadgetState(#DEBUG_CHK_BOLD)
                              gView\cells(stIdx)\gadgets(0)\italic = GetGadgetState(#DEBUG_CHK_ITALIC)
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_NUDGE_L
                        ; Nudge left - uncheck right (mutually exclusive)
                        If GetGadgetState(#DEBUG_CHK_NUDGE_L)
                           SetGadgetState(#DEBUG_CHK_NUDGE_R, #False)
                        EndIf
                        If gSelectStartCol >= 0
                           Protected nlIdx.i = GetCellIndex(gSelectStartCol, gSelectStartRow)
                           If gView\cells(nlIdx)\gadgetCount > 0
                              Protected nlAlign.i = gView\cells(nlIdx)\gadgets(0)\align & $0F
                              If GetGadgetState(#DEBUG_CHK_NUDGE_L) : nlAlign | #LJV_TOLEFT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_R) : nlAlign | #LJV_TORIGHT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_U) : nlAlign | #LJV_UP : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_D) : nlAlign | #LJV_DOWN : EndIf
                              gView\cells(nlIdx)\gadgets(0)\align = nlAlign
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_NUDGE_R
                        ; Nudge right - uncheck left (mutually exclusive)
                        If GetGadgetState(#DEBUG_CHK_NUDGE_R)
                           SetGadgetState(#DEBUG_CHK_NUDGE_L, #False)
                        EndIf
                        If gSelectStartCol >= 0
                           Protected nrIdx.i = GetCellIndex(gSelectStartCol, gSelectStartRow)
                           If gView\cells(nrIdx)\gadgetCount > 0
                              Protected nrAlign.i = gView\cells(nrIdx)\gadgets(0)\align & $0F
                              If GetGadgetState(#DEBUG_CHK_NUDGE_L) : nrAlign | #LJV_TOLEFT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_R) : nrAlign | #LJV_TORIGHT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_U) : nrAlign | #LJV_UP : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_D) : nrAlign | #LJV_DOWN : EndIf
                              gView\cells(nrIdx)\gadgets(0)\align = nrAlign
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_NUDGE_U
                        ; Nudge up - uncheck down (mutually exclusive)
                        If GetGadgetState(#DEBUG_CHK_NUDGE_U)
                           SetGadgetState(#DEBUG_CHK_NUDGE_D, #False)
                        EndIf
                        If gSelectStartCol >= 0
                           Protected nuIdx.i = GetCellIndex(gSelectStartCol, gSelectStartRow)
                           If gView\cells(nuIdx)\gadgetCount > 0
                              Protected nuAlign.i = gView\cells(nuIdx)\gadgets(0)\align & $0F
                              If GetGadgetState(#DEBUG_CHK_NUDGE_L) : nuAlign | #LJV_TOLEFT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_R) : nuAlign | #LJV_TORIGHT : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_U) : nuAlign | #LJV_UP : EndIf
                              If GetGadgetState(#DEBUG_CHK_NUDGE_D) : nuAlign | #LJV_DOWN : EndIf
                              gView\cells(nuIdx)\gadgets(0)\align = nuAlign
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_NUDGE_D
                        ; Nudge down - uncheck up (mutually exclusive)
                        If GetGadgetState(#DEBUG_CHK_NUDGE_D)
                           SetGadgetState(#DEBUG_CHK_NUDGE_U, #False)
                        EndIf
                        If gSelectStartCol >= 0
                           Protected ndIdx.i = GetCellIndex(gSelectStartCol, gSelectStartRow)
                           If gView\cells(ndIdx)\gadgetCount > 0
                              Protected ndAlign.i = gView\cells(ndIdx)\gadgets(0)\align & $0F
                              ...
                              gView\cells(ndIdx)\gadgets(0)\align = ndAlign
                           EndIf
                           ViewShow(gView\width, gView\height)
                           UpdateCodeDisplay()
                        EndIf
                     ...
                     Case #DEBUG_CHK_EVT_TAP, #DEBUG_CHK_EVT_SWIPE, #DEBUG_CHK_EVT_DRAG, #DEBUG_CHK_EVT_LONG
                        ...
                        If gSelectStartCol >= 0
                           ...
                           If gView\cells(evIdx)\gadgetCount > 0
                              ...
                              gView\cells(evIdx)\gadgets(0)\events = newEvts
                              UpdateCodeDisplay()
                           EndIf
                        EndIf
                        ProcedureReturn #True
                     ...
                     Case #DEBUG_INPUT_HANDLER
                        ...
                        If gSelectStartCol >= 0
                           ...
                           If gView\cells(hIdx)\gadgetCount > 0
                              gView\cells(hIdx)\gadgets(0)\handler = GetGadgetText(#DEBUG_INPUT_HANDLER)
                              UpdateCodeDisplay()
                           EndIf
                        EndIf
                        ProcedureReturn #True
