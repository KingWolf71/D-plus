...
Procedure               C2JZ()
   ...
   sp - 1
   CompilerIf #DEBUG
      ; V1.020.094: Debug JZ in quicksort function range (pc 292-320) at depth 6
      If gStackDepth >= 6 And pc >= 292 And pc <= 320
...
Procedure               C2CALL()
   ...
   Protected localSlotStart.l, arrayOffset.l, actualSlot.l
   ...
   ; Copy parameters from evaluation stack to allocated gVar[] slots
   If nParams > 0
      paramSp = sp - nParams
      For i = 0 To nParams - 1
         ...
         CompilerIf #DEBUG
            Debug "  Param[" + Str(i) + "]: gVar(" + Str(paramSp + i) + ")=" + Str(gVar(paramSp + i)\i) + " -> localSlot(" + Str(localSlotStart + i) + ")"
         CompilerEndIf
         ; V1.18.63.9: Always copy pointer metadata (clears stale values if not a pointer)
         gVar(localSlotStart + i)\ptr = gVar(paramSp + i)\ptr
         gVar(localSlotStart + i)\ptrtype = gVar(paramSp + i)\ptrtype
      Next
   EndIf
...
XIncludeFile "c2-builtins-v01.pbi"

;- Include Array Operations Module
XIncludeFile "c2-arrays-v01.pbi"
